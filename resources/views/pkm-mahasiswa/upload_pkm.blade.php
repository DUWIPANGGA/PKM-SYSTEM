    ...
    <form action="{{ route('pkm.submit') }}" method="POST" enctype="multipart/form-data">
        ...
        @if (!is_null($data_pengajuan))
                    ...
                    <div class="anggota-row d-flex align-items-center gap-2 mb-3">
                        ...
                        <select name="anggota[{{ $count }}][prodi]" value="{{ $item['prodi'] }}" class="form-select"
                            ...
                            <option value="Teknik Informatika"
                                {{ $item['prodi'] == 'Teknik Informatika' ? 'selected' : '' }}>Teknik Informatika</option>
            ...
            <div class="mb-3">
                ...
                <input type="text" id="jangka_waktu" name="jangka_waktu" class="form-control" value="{{$data_pengajuan->jangka_waktu  }}" required>
            </div>
            <div class="mb-3">
                <label for="dana">Dana Pengajuan:</label>
                <input type="text" id="dana" name="dana" placeholder="*Isi sesuai proposal"
                    class="form-control" value="{{ $data_pengajuan->dana }}" required>
            </div>
            <div class="mb-3">
                ...
                <input type="file" id="proposal_file" name="proposal_file" accept=".pdf" class="form-control"
                    value="{{ asset('/public' . $data_pengajuan->proposal_file) }}" required>
        ...
        @else
            ...
            <div id="anggota-section">
                <div class="anggota-row d-flex align-items-center gap-2 mb-3">
                    ...
                    <input type="text" name="anggota[0][nim]" placeholder="NIM" class="form-control" value=""
                        required>
                    <input type="text" name="anggota[0][nama]" placeholder="Nama" class="form-control"
                        value="" required>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    <label for="tahun_usulan" class="form-label">Tahun Usulan</label>
                    <input type="text" name="tahun_usulan" id="tahun_usulan" class="form-control" value="{{ old('tahun_usulan') }}"
                        placeholder="Masukkan Tahun Usulan">
                </div>
            ...
            <div class="mb-3">
                ...
                <select name="jenis_pkm" id="jenis_pkm" class="form-control">
                    <option value=""><b>Pilih Jenis PKM</b></option>
                    <option value="PKM-RE" {{ old('jenis_pkm') == 'PKM-RE' ? 'selected' : '' }}>PKM Riset Eksakta (PKM-RE)</option>
                    <option value="PKM-RSH" {{ old('jenis_pkm') == 'PKM-RSH' ? 'selected' : '' }}>PKM Riset Sosial Humaniora (PKM-RSH)</option>
                    <option value="PKM-PI" {{ old('jenis_pkm') == 'PKM-PI' ? 'selected' : '' }}>PKM Penerapan IPTEK (PKM-PI)</option>
                    <option value="PKM-K" {{ old('jenis_pkm') == 'PKM-K' ? 'selected' : '' }}>PKM Kewirausahaan (PKM-K)</option>
                    <option value="PKM-KI" {{ old('jenis_pkm') == 'PKM-KI' ? 'selected' : '' }}>PKM Karya Inovatif (PKM-KI)</option>
                    <option value="PKM-VGK" {{ old('jenis_pkm') == 'PKM-VGK' ? 'selected' : '' }}>PKM Video Gagasan Konstruktif (PKM-VGK)</option>
                    <option value="PKM-AI" {{ old('jenis_pkm') == 'PKM-AI' ? 'selected' : '' }}>PKM Artikel Ilmiah (PKM-AI)</option>
                    <option value="PKM-PM" {{ old('jenis_pkm') == 'PKM-PM' ? 'selected' : '' }}>PKM Pengabdian kepada Masyarakat (PKM-PM)</option>
                    <option value="PKM-KC" {{ old('jenis_pkm') == 'PKM-KC' ? 'selected' : '' }}>PKM Karsa Cipta (PKM-KC)</option>
                    <option value="PKM-GFT" {{ old('jenis_pkm') == 'PKM-GFT' ? 'selected' : '' }}>PKM Gagasan Futuristik Tertulis (PKM-GFT)</option>
                </select>
            </div>
            ...
            <div class="mb-3">
                <label for="jangka_waktu">Jangka Waktu Pelaksanaan:</label>
                <input type="text" id="jangka_waktu" name="jangka_waktu" class="form-control" value="{{ old('jangka_waktu') }}"
                    required>
            </div>
            <div class="mb-3">
                <label for="dana">Dana Pengajuan:</label>
                <input type="text" id="dana" name="dana" placeholder="*Isi sesuai proposal"
                    class="form-control" value="{{ old('dana') }}" required>
            </div>
            <div class="mb-3">
                <label for="proposal_file">File Upload (PDF saja):</label>
                <input type="file" id="proposal_file" name="proposal_file" accept=".pdf" class="form-control"
